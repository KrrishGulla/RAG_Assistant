...
window.onload = () => {
    ...
    fetchIceToken();
    setInterval(fetchIceToken, 60 * 1000);

    // Automatically start avatar session after short delay
    setTimeout(() => {
        document.getElementById('startSession').click();
    }, 1500); // Delay to ensure tokens load first
};
...
// Listen for incoming assistant answer from parent iframe
window.addEventListener("message", (event) => {
    if (!event.data || event.data.type !== "injectAndSpeak") return;

    const spokenText = event.data.payload;
    const spokenTextBox = document.getElementById("spokenText");

    if (spokenTextBox) {
        spokenTextBox.value = spokenText;

        // Make sure avatar is ready before calling speak
        setTimeout(() => {
            try {
        ...
        }, 300); // Short delay to ensure DOM is stable
    }
});

// ✅ Listen for messages from parent window (Assistant UI)
window.addEventListener("message", (event) => {
    if (event.data && event.data.type === "speak") {
        ...
        if (spokenBox) {
            spokenBox.value = text;
        }

        // Speak after short delay if avatar is ready
        setTimeout(() => {
